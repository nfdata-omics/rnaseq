...
option_list <- list(
    make_option(c("-F", "--FDR"), action="store", type="double", default=0.25,
        help=paste0("The FDR cutoff to define significant pathways. Results (table and GSEA curve) ",
        "will be produced for significant pathways only. [default \"%default\"]")),
    make_option(c("-r", "--ranking"), action="store", type="character", default="logFC",
        help=paste0("The ranking statistics used to order genes. Possible options are 'logFC' ",
        "or 'pvalue'. In case pvalue is selected, -log10(pval) will be multiplied by the sign of ",
        "log2FC. [default \"%default\"]")),
    make_option(c("-m", "--min"), action="store", type="integer", default=10,
        help=paste0("Minimal size of the gene sets included in the analysis. [default \"%default\"]")),
    make_option(c("-M", "--max"), action="store", type="integer", default=500,
        help=paste0("Maximal size of the gene sets included in the analysis. [default \"%default\"]")),
    make_option(c("-v", "--version"), action="store_true", default=FALSE,
        help="Print the list of loaded package versions and exit.")
)

parser <- OptionParser(
    usage = "%prog [options] dge_toptable gmt_file",
    option_list = option_list, prog = "gsea",
    description = "
    Perform Preranked GSEA using the clusterProfiler R package.
    'dge_toptable' is the path of the DGE toptable from which genes are sorted
    according to the ranking statistic. The DGE toptable must have the following structure:
    .META: dge_toptable
        1. gene names
    ...
    'gmt_file' is the path of a .gmt file containing pathways to evaluate.
    Each pathway must be reported in a different row, with genes separated by tabs."
)
...
my_gsea <- GSEA(my_rnk, TERM2GENE=my_gmt, pvalueCutoff=fdr,
    minGSSize=mingenes, maxGSSize=maxgenes)
...
if (nrow(curated_gsea) > 0) {
  ...
  no_enrichment = data.frame(empty=paste("No significant enrichment was found at level FDR<", fdr, sep=""))
